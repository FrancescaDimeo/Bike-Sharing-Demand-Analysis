...
get_weather_forecaset_by_cities <- function(city_names){
  ...
  # Soluzione: usiamo solo i primi 196 dati di ogni lista per farli combaciare
  weather_df <- tibble(
    CITY_ASCII = head(city, 196), 
    WEATHER = head(weather, 196), 
    TEMPERATURE = head(temperature, 196),
    VISIBILITY = head(visibility, 196), 
    HUMIDITY = head(humidity, 196), 
    WIND_SPEED = head(wind_speed, 196), 
    SEASONS = head(season, 196), 
    HOURS = head(hours, 196), 
    FORECASTDATETIME = head(forecast_date, 196), 
    LABEL = head(weather_labels, 196), 
    DETAILED_LABEL = head(weather_details_labels, 196)
  )
